...
if False:
    ...
    for _ in range(20):
        ...
        data_stat = trace_trust_region[-1]
...
mpc_param = dict(x_lb=None, x_ub=None, u_lb=data_stat['u_lb'], u_ub=data_stat['u_ub'],
                 cp_param=env.get_cost_param(), cf_param=env.get_cost_param())
print('Target cube angle:', env.get_cost_param(), '\n')

# render
rollouts = rollout_mpcReceding(env=env, rollout_horizon=rollout_horizon,
                               mpc=mpc, mpc_aux=dyn_aux,
                               mpc_param=mpc_param,
                               render=False, debug_mode=False, print_lam=True)
...
if True:
    analyser.plot_state_lam3(dict(state_traj=state_traj, lam_traj=model_lam_traj),
                             state_dim_ref=dict(ref_dim=0, ref_val=env.get_cost_param()))
