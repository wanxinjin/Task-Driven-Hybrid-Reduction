...
save_data_name = 'task1_fixed_target_seed200'
saved_info = dict()

#  ---------------------------- load mujoco trifinger env ------------
env = TriFingerQuasiStaticGroundRotateEnv()
# fixed target
env.target_cube_angle = np.random.uniform(low=0.6, high=0.6, size=(500,))
# gaussian target
# env.target_cube_angle = np.random.normal(loc=0.6, scale=0.2, size=(500,))
